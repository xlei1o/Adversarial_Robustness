...
model and adds to it adversarial robustness functionalities as well as other useful
...
class Attacker(ch.nn.Module):
    ...
    def __init__(self, model):
        ...
        Args:
            nn.Module model : the PyTorch model to robustness
    ...
    def forward(self, x, target, *_, constraint, eps, step_size, iterations,
        ...
        Args:
            ...
            random_start (bool) : if True, start the robustness with a random step.
            random_restarts (bool) : if True, do many random restarts and
                take the worst robustness (in terms of loss) per input.
            do_tqdm (bool) : if True, show a tqdm progress bar for the robustness.
            ...
            custom_loss (function|None) : if provided, used instead of the
                ...
                adversarial robustness. The function should take in
            ...
            use_best (bool) : If True, use the best (in terms of loss)
                iterate of the robustness process instead of just the last one.
        ...
        # Random restarts: repeat the robustness and find the worst-case
...
class AttackerModel(ch.nn.Module):
    ...
    for convenient access to adversarial attacks and other applications.::
        ...
        out, new_im = model(x, y, make_adv=True) # adversarial robustness
        out, new_im = model(x, y, make_adv=True, targeted=True) # targeted robustness
    ...
    def forward(self, inp, target=None, make_adv=False, with_latent=False,
        ...
        Parameters:
            ...
            target (ch.tensor) : ignored if `make_adv == False`. Otherwise,
                labels for adversarial robustness.
